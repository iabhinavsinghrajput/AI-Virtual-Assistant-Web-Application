...
export const askToAssistant=async (req,res)=>{
   try {
      ...
      if (!result) {
         return res.status(500).json({ response: "I am having trouble connecting to my brain right now. The API might be rate limited." })
      }
      
      if (result.error) {
         return res.status(500).json({ response: `Google API Error: ${result.error}` })
      }
